...
public class DBExporter implements IModelExporter {
						...
						if ( (objectToExport instanceof IIdentifier) && (objectToExport instanceof INameable) ) {	// we only export components that have got an ID and a name
							...
							//case "UsedByRelationship" :				// Archi 3
								...
								if ( dbSelectModel.getDbLanguage().equals("SQL") ) {
									...
									DBPlugin.request(db, "MATCH (m:model {model:?, version:?}), (source {id:?})-[:isInModel]->(m), (target {id:?})-[:isInModel]->(m) CREATE (source)-[new:"+relationship.getClass().getSimpleName()+" {id:?, name:?, type:?, documentation:?, folder:?, accesstype:?, strength:?}]->(target)",
											projectId,
											version,
											relationship.getSource().getId(),
											relationship.getTarget().getId(),
											...
											relationship.getDocumentation(),
											((IIdentifier)relationship.eContainer()).getId(),
											objectToExport.eClass().getName() == "AccessRelationship" ? ((IAccessRelationship)objectToExport).getAccessType() : -1,
											objectToExport.eClass().getName() == "InfluenceRelationship" ? ((IInfluenceRelationship)objectToExport).getStrength() : ""
											);
								}
	...
	private void exportDiagramModelObject(String _parentId, IDiagramModelObject diagramModelObject, int _rank, int _indent) throws Exception {
		...
		case "DiagramModelArchimateObject" :
			...
			textPosition = diagraModelArchimateObject.getTextPosition();
	...
	private void exportConnection(String _parentId, IDiagramModelConnection diagramModelConnection, int _rank) throws Exception {
		...
		if ( dbSelectModel.getDbLanguage().equals("SQL") ) {
			DBPlugin.insert(db, "INSERT INTO connection (id, model, version, class, name, documentation, islocked, font, fontcolor, linecolor, linewidth, parent, relationship, source, target, text, textposition, type, rank)",
					...
					diagramModelConnection.getName(),						// Archi 4
			...
			DBPlugin.request(db, "MATCH (m:model {model:?, version:?}) CREATE (new:connection {id:?, class:?, name:?, documentation:?, islocked:?, font:?, fontcolor:?, linecolor:?, linewidth:?, parent=?, relationship:?, source:?, target:?, text:?, textposition:?, type:?, rank:?}), (new)-[:isInModel]->(m)",
					DBPlugin.getProjectId(diagramModelConnection.getId()),
					DBPlugin.getVersion(diagramModelConnection.getId()),
					DBPlugin.getId(diagramModelConnection.getId()),
					diagramModelConnection.eClass().getName(),
	...
	private void exportDiagramModelReference(String _parentId, IDiagramModelReference _diagramModelReference, int _rank, int _indent) throws Exception {
		...
		if ( dbSelectModel.getDbLanguage().equals("SQL") ) {
			DBPlugin.insert(db, "INSERT INTO diagrammodelreference (id, model, version, parent, fillcolor, font, fontcolor, linecolor, linewidth, textalignment, targetconnections, x, y, width, height, rank, indent)",
			...
			DBPlugin.request(db, "MATCH (m:model {model:?, version:?}) CREATE (new:diagrammodelreference {id:?, parent:?, fillcolor:?, font:?, fontcolor:?, linecolor:?, linewidth:?, textalignment:?, targetconnections:?, x:?, y:?, width:?, height, rank:?, indent:?}), (new)-[:isInModel]->(m)",
					DBPlugin.getProjectId(_diagramModelReference.getId()),
					DBPlugin.getVersion(_diagramModelReference.getId()),
					DBPlugin.getId(_diagramModelReference.getId()),
					_parentId,
