...
public class DBImporter implements IModelImporter, ISelectedModelImporter {
	...
	private DBLogger logger = new DBLogger(DBImporter.class);
	...
	public void doImport(IArchimateModel notUsedModel) throws IOException {
		...
		for ( HashMap<String, String> modelSelected: selectedModels ) {
			...
			// we compare the ID of the model to import with the ID of existing models
		...
		for ( HashMap<String, String> modelSelected: selectedModels ) {
			try {
				...
				if ( modelSelected.get("_import_it").equals("yes") ) {
					...
					if ( logger.isDebugEnabled() ) logger.debug("Please wait while counting and versionning components ...");
					...
					if ( logger.isDebugEnabled() ) logger.debug("Please wait while importing components ...");
	...
	private void importArchimateElement(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("Importing archimateElements");
		...
		while(result.next()) {
			if ( logger.isDebugEnabled() ) logger.debug( "Importing "+result.getString("type")+" id="+result.getString("id")+" name="+result.getString("name"));
	...
	private void importRelationship(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing relationships");
		...
		while(result.next()) {
			if ( logger.isDebugEnabled() ) logger.debug( "Importing "+result.getString("type")+" id="+result.getString("id")+" source="+result.getString("source")+" target="+result.getString("target"));
	...
	private void importArchimateDiagramModel(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing archimateDiagramModels");
	...
	private void importDiagramModelArchimateObject(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing diagramModelArchimateObjects");
		...
		while(result.next()) {
			if ( logger.isDebugEnabled() ) logger.debug( "Importing "+result.getString("class")+" id="+result.getString("id")+" name="+result.getString("name"));
	...
	private void importConnections(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("import connections");
		...
		while(result.next()) {
			if ( logger.isDebugEnabled() ) logger.debug( "Importing "+result.getString("class")+" id="+result.getString("id")+" source="+result.getString("source")+" target="+result.getString("target"));
	...
	private void importCanvasModel(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing canvasModels");
		...
		while(result.next()) {
			if ( logger.isDebugEnabled() ) logger.debug( "Importing CanvasModel id="+result.getString("id")+" name="+result.getString("name"));
	...
	private void importCanvasModelBlock(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing canvasModelBlock");
		...
		while(result.next()) {
			if ( logger.isDebugEnabled() ) logger.debug( "Importing CanvasModelBlock id="+result.getString("id")+" name="+result.getString("name"));
	...
	private void importCanvasModelSticky(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing canvasModelSticky");
		...
		while(result.next()) {
			if ( logger.isDebugEnabled() ) logger.debug( "Importing CanvasModelSticky id="+result.getString("id")+" name="+result.getString("name"));
	...
	private void importCanvasModelImage(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing canvasModelImage");
		...
		while(result.next()) {
			if ( logger.isDebugEnabled() ) logger.debug( "Importing CanvasModelImage id="+result.getString("id")+" name="+result.getString("name"));
	...
	private void importDiagramModelReference(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing diagramModelReference");
		...
		while(result.next()) {
			if ( logger.isDebugEnabled() ) logger.debug( "Importing DiagramModelReference id="+result.getString("id"));
	...
	private void importSketchModel(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing sketchModels");
		...
		while(result.next()) {
			if ( logger.isDebugEnabled() ) logger.debug( "Importing SketchModel id="+result.getString("id")+" name="+result.getString("name"));
	...
	private void importSketchModelActor(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing sketchModelActors");
		...
		while(result.next()) {
			if ( logger.isDebugEnabled() ) logger.debug( "Importing SketchModelActor id="+result.getString("id")+" name="+result.getString("name"));
	...
	private void importSketchModelSticky(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing sketchModelSticky");
		...
		while(result.next()) {
			if ( logger.isDebugEnabled() ) logger.debug( "Importing SketchModelSticky id="+result.getString("id")+" name="+result.getString("name"));
	...
	private void importFolders(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing folders");
		...
		while(result.next()) {
			if ( result.getInt("type") != 0 ) {				// top level folders have been created at the same time than the model 
				...
				if ( logger.isDebugEnabled() ) logger.debug( "Using default first level folder \""+result.getString("name")+"\" (type="+result.getInt("type")+", id="+folder.getId()+")");
			} else {
				...
				if ( result.getString("parent") == null ) { // top level folder that has not been created at the same time than the model ("derived" or "deleted" for instance)
					if ( logger.isDebugEnabled() ) logger.debug( "Importing first level folder \""+result.getString("name")+"\" (type="+result.getInt("type")+", id="+folder.getId()+", , parent="+(result.getString("parent")==null?"null":"\""+result.getString("parent")+"\"")+")");
					_dbModel.getFolders().add(folder);
				} else {
					if ( logger.isDebugEnabled() ) logger.debug( "Importing subfolder \""+result.getString("name")+"\" (type="+result.getInt("type")+", id="+folder.getId()+", parent="+(result.getString("parent")==null?"null":"\""+result.getString("parent")+"\"")+")");
	...
	private void importImages(DBModel _dbModel) throws Exception {
		if ( logger.isDebugEnabled() ) logger.debug("importing images");
		
		// we import only required images
		for (String path: _dbModel.getImagePaths() ) {
			if ( logger.isDebugEnabled() ) logger.debug( "Searching in database for image path "+path);
