...
public class DBGui {
	...
	protected DBGui(String title) {
		...
		this.dialog.addListener(SWT.Close, new Listener()
		{
			...
			public void handleEvent(Event event)
			{
				...
				if ( DBGui.this.btnClose.getText().equals("Cancel") ) {
					doIt = DBGuiUtils.question("Are you sure you wish to cancel ?");
				}
				...
				if ( doIt ) {
					...
					try {
						...
						DBGuiUtils.popup(Level.ERROR, "Failed to rollback and close the database connection.", e);
					}
					close();
					event.doit = true;
				}
		...
		this.btnSetPreferences.addSelectionListener(new SelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent event) { try { setPreferences(); } catch (Exception e) { DBGuiUtils.popup(Level.ERROR, "Failed to set preferences", e); } }
			@Override
	...
	protected void setPreferences() {
		...
		if ( prefDialog.open() == 0 ) {
			...
			if ( this.comboDatabases.getItemCount() == 0 )
				DBGuiUtils.popup(Level.ERROR, "You won't be able to export until a database is configured in the preferences.");
		}
	...
	protected void databaseSelected() {
		...
		try {
			...
			DBGuiUtils.popup(Level.ERROR, "Cannot connect to the database.", err);
			return;
		}
		...
		try {
			...
			closeMessage();
			DBGuiUtils.popup(Level.ERROR, "Cannot use this database.", err);
			notConnectedToDatabase();
			return;
		} finally {
			closeMessage();
	...
	protected Boolean fillInCompareTable(Tree tree, TreeItem treeItem, EObject memoryObject, int memoryObjectversion) {
		...
		try {
			...
			DBGuiUtils.popup(Level.ERROR, "Failed to get component "+dbMetadata.getDebugName()+" from the database.", err);
			return null;
		}
